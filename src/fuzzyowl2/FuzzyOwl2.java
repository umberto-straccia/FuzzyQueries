...
public class FuzzyOwl2
{
	...
	protected HashSet<String> processedAxioms;
	...
	public FuzzyOwl2(String input, String output)
	{
		...
		ontologyPath = input;
		processedAxioms = new HashSet<String> ();
	...
	protected void loadOntology(String ontologyPath)
	{
		...
		catch (Exception e)
		{
			...
			catch (Exception e2)
			{
				...
				if (message.contains("URI is not absolute"))
	...
	protected void processOntologyAxioms()
	{
		...
		{
/*
			System.out.println("Forgetting about the following axioms:");	
			System.out.println("\tANNOTATION_ASSERTION: " + o.getAxioms(AxiomType.ANNOTATION_ASSERTION).size());
			System.out.println("\tANNOTATION_PROPERTY_DOMAIN: " + o.getAxioms(AxiomType.ANNOTATION_PROPERTY_DOMAIN).size());
			System.out.println("\tANNOTATION_PROPERTY_RANGE�: " + o.getAxioms(AxiomType.ANNOTATION_PROPERTY_RANGE).size());
			System.out.println("\tDATATYPE_DEFINITION: " + o.getAxioms(AxiomType.DATATYPE_DEFINITION).size());
			System.out.println("\tDECLARATION: " + o.getAxioms(AxiomType.DECLARATION).size());
			System.out.println("\tHAS_KEY�: " + o.getAxioms(AxiomType.HAS_KEY).size());
			System.out.println("\tSWRL_RULE�: " + o.getAxioms(AxiomType.SWRL_RULE).size());
*/
			...
			{
				if ((c.isTopEntity() == false) && (! processedAxioms.contains(c.toString())))
				{
					writeConceptDeclaration(c);
					processedAxioms.add(c.toString());
				}
			}

			for (OWLDataPropertyExpression dp : o.getDataPropertiesInSignature())
			{
				if (! processedAxioms.contains(dp.toString()))
				{
					writeDataPropertyDeclaration(dp);
					processedAxioms.add(dp.toString());
				}
			}

			for (OWLObjectPropertyExpression op : o.getObjectPropertiesInSignature())
			{
				if (! processedAxioms.contains(op.toString()))
				{
					writeObjectPropertyDeclaration(op);
					processedAxioms.add(op.toString());
				}
			}
			...
			for (OWLSubClassOfAxiom ax : o.getAxioms(AxiomType.SUBCLASS_OF))
			{
				...
				if (d != 1)
				{
					...
					processedAxioms.add(subclass.toString() + " => " + superclass.toString() );
				}
			}
			...
			for (OWLSubObjectPropertyOfAxiom ax : o.getAxioms(AxiomType.SUB_OBJECT_PROPERTY))
			{
				...
				if (d != 1)
				{
					...
					processedAxioms.add(subProperty.toString() + " => " + superProperty.toString() );
				}
			}
			...
			for (OWLSubPropertyChainOfAxiom ax : o.getAxioms(AxiomType.SUB_PROPERTY_CHAIN_OF))
			{
				...
				if (d != 1)
				{
					...
					processedAxioms.add(chain.toString() + " => " + superProperty.toString() );
				}
			}
			...
			for (OWLClassAssertionAxiom ax : o.getAxioms(AxiomType.CLASS_ASSERTION))
			{
				...
				if (d != 1)
				{
					...
					processedAxioms.add(i.toString() + " : " + c.toString());
				}
			}
			...
			for (OWLObjectPropertyAssertionAxiom ax : o.getAxioms(AxiomType.OBJECT_PROPERTY_ASSERTION))
			{
				...
				if (d != 1)
				{
					...
					processedAxioms.add("(" + i1.toString() +", " + i2.toString() + ") : " + p.toString() );
				}
			}
			...
			for (OWLNegativeObjectPropertyAssertionAxiom ax : o.getAxioms(AxiomType.NEGATIVE_OBJECT_PROPERTY_ASSERTION))
			{
				...
				if (d != 1)
				{
					...
					processedAxioms.add("(" + i1.toString() +", " + i2.toString() + ") : not " + p.toString() );
				}
			}
		...
		for(OWLOntology o : ontologies)
		{
			for (OWLSubClassOfAxiom ax : o.getAxioms(AxiomType.SUBCLASS_OF))
			{
				...
				OWLClassExpression superclass = ax.getSuperClass();
				double d = getDegree(ax);
				if ( (d == 1) && (! processedAxioms.contains(subclass.toString() + " => " + superclass.toString() ) ) )
				{
					writeSubclassOfAxiom(subclass, superclass, 1);
					processedAxioms.add(subclass.toString() + " => " + superclass.toString() );
				}
			}
			...
			for (OWLSubObjectPropertyOfAxiom ax : o.getAxioms(AxiomType.SUB_OBJECT_PROPERTY))
			{
				...
				OWLObjectPropertyExpression superProperty = ax.getSuperProperty();
				double d = getDegree(ax);
				if ( (d == 1) && (! processedAxioms.contains(subProperty.toString() + " => " + superProperty.toString() ) ) )
				{
					writeSubObjectPropertyOfAxiom(subProperty, superProperty, d);
					processedAxioms.add(subProperty.toString() + " => " + superProperty.toString());
				}
			...
			for (OWLSubPropertyChainOfAxiom ax : o.getAxioms(AxiomType.SUB_PROPERTY_CHAIN_OF))
			{
				...
				OWLObjectPropertyExpression superProperty = ax.getSuperProperty();
				double d = getDegree(ax);
				if ( (d == 1) && (! processedAxioms.contains(chain.toString() + " => " + superProperty.toString() ) ) )
				{
					writeSubPropertyChainOfAxiom(chain, superProperty, d);
					processedAxioms.add(chain.toString() + " => " + superProperty.toString());
				}
			...
			for (OWLClassAssertionAxiom ax : o.getAxioms(AxiomType.CLASS_ASSERTION))
			{
				...
				OWLIndividual i = ax.getIndividual();
				double d = getDegree(ax);
				if ( (d == 1) && (! processedAxioms.contains(i.toString() + " : " + c.toString() ) ) )
				{
					writeConceptAssertionAxiom(i, c, d);
					processedAxioms.add(i.toString() + " : " + c.toString());
				}
			...
			for (OWLObjectPropertyAssertionAxiom ax : o.getAxioms(AxiomType.OBJECT_PROPERTY_ASSERTION))
			{
				...
				OWLIndividual i2 = ax.getObject();
				double d = getDegree(ax);
				if ( (d == 1) && (! processedAxioms.contains("(" + i1.toString() +", " + i2.toString() + ") : " + p.toString() ) ) )
				{
					writeObjectPropertyAssertionAxiom(i1, i2, p, d);
					processedAxioms.add("(" + i1.toString() +", " + i2.toString() + ") : " + p.toString() );
				}
			...
			for (OWLNegativeObjectPropertyAssertionAxiom ax : o.getAxioms(AxiomType.NEGATIVE_OBJECT_PROPERTY_ASSERTION))
			{
				...
				OWLIndividual i2 = ax.getObject();
				double d = getDegree(ax);
				if ( (d == 1) && (! processedAxioms.contains("(" + i1.toString() +", " + i2.toString() + ") : not " + p.toString() ) ) )
				{
					writeNegativeObjectPropertyAssertionAxiom(i1, i2, p, d);
					processedAxioms.add("(" + i1.toString() +", " + i2.toString() + ") : not " + p.toString() );
				}
	 ...
	 */
	protected String getClassName(OWLClassExpression c)
	{
	...
	protected String getObjectPropertyName(OWLObjectPropertyExpression p)
	{
		...
		else if (p.isAnonymous())
		{
			...
			name = name.substring(pos + 1, name.length() - 2);
	...
	/**
	 * Gets the short name (without namespaces) of an OWL 2 entity given its name.
	 * @param aux a The name of the entity.
	 * @return Short name of entity.
	 */
	protected String getShortName(String aux)
	{
		return aux;
	}
